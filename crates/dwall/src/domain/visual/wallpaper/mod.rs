...
use crate::{DwallResult, domain::time::solar_calculator::SolarAngle};
...
///
/// Implementations can set wallpapers using various platform APIs:
/// - Windows IDesktopWallpaper COM interface
/// - Linux desktop environment specific APIs
pub trait WallpaperProvider {
    /// Sets the wallpaper for a specific monitor
    ///
    /// # Arguments
    /// * `monitor_id` - The unique identifier of the monitor
    /// * `image_path` - The path to the wallpaper image file
    fn set_monitor_wallpaper(&self, monitor_id: &str, image_path: &Path) -> DwallResult<()>;

    /// Sets the lock screen image
    ///
    /// # Arguments
    /// * `image_path` - The path to the lock screen image file
